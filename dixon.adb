--  Dixon's factorization method — Ada 2023 body (educational toy).
...
is
   ...
   function Dixon_Factor_Base (B : U64) return Factor_Base is
   begin
      return Primes_Up_To (B);
   end Dixon_Factor_Base;
   ...
   ------------------------------------------------------------------
   --  Tiny LCG for reproducible "random" A values
   ------------------------------------------------------------------

   procedure Lcg_Next (State : in out U64) is
   begin
      --  Knuth MMIX multiplicative LCG (mod 2^64 via wraparound).
      State := State * 6364136223846793005 + 1442695040888963407;
   end Lcg_Next;

   function Random_A (State : in out U64; N : U64) return U64 is
      Span : U64;
   begin
      Lcg_Next (State);
      if N <= 2 then
      ...
      Span := N - 2;  --  map into [1, N-1]
      return 1 + (State rem Span);
   end Random_A;
      ...
      begin
         for I in 1 .. Rows loop
            declare
               ...
               A2  : constant U64 := Mul_Mod (Rel.A, Rel.A, N);
            begin
               if Rel.B /= A2 then
                  raise Invalid_Argument;
               end if;
               if not Is_B_Smooth (Rel.B, Base) then
                  raise Invalid_Argument;
               end if;
               Exps (I) := Smooth_Exponents (Rel.B, Base);
   ...
   ------------------------------------------------------------------
   --  Factor_Dixon — random squares + CoS
   ------------------------------------------------------------------
   ...
   begin
      ...
      declare
         ...
         Base      : constant Factor_Base := Dixon_Factor_Base (B);
         Need      : constant Natural :=
           Natural'Min (Max_FB, Base'Length + 8);
         Buf       : array (1 .. Max_FB) of Relation;
         Count     : Natural := 0;
         State     : U64 := (if Seed = 0 then 1 else Seed);
         Max_Tries : constant Natural := 200_000;
         Tries     : Natural := 0;
         Factor    : U64;
         Seen_A    : array (1 .. Max_FB) of U64 := [others => 0];
      begin
         if Base'Length = 0 then
            return Smallest_Prime_Factor (N);
         end if;

         while Count < Need and then Tries < Max_Tries loop
            Tries := Tries + 1;
            declare
               A : constant U64 := Random_A (State, N);
               Q : constant U64 := Mul_Mod (A, A, N);
               Dup : Boolean := False;
            begin
               if Q = 0 then
                  declare
                     G : constant U64 := Gcd (A, N);
                  begin
                     if G > 1 and then G < N then
                        return G;
                     end if;
   ...
   is
   begin
      return Factor_Dixon (N, Smoothness, Seed);
   end Factor;
